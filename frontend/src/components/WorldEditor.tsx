...
interface Props {
  visible: boolean;
  initial?: { id?: number; name?: string; description?: string; authorNote?: string; settingDetails?: string; storyDetails?: string; lorebookUuids?: string[] };
  onClose: () => void;
  onSave: (payload: { name: string; description?: string; authorNote?: string; settingDetails?: string; storyDetails?: string; lorebookUuids?: string[] }) => void;
}
...
export default function WorldEditor({ visible, initial, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    setSettingDetails(initial?.settingDetails || '');
    ...
    <div className="ef-modal-overlay" onMouseDown={onClose}>
      <div className="ef-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div className="ef-modal-body">
          ...
          <label className="ef-label">Setting Details</label>
          <textarea className="ef-textarea" value={settingDetails} onChange={(e) => setSettingDetails(e.target.value)} />
        ...
        <div className="ef-modal-footer">
          ...
          <button
            ...
            onClick={() => onSave({ name, description, authorNote, settingDetails, storyDetails, lorebookUuids: selectedLorebooks })}
            disabled={!name.trim()}
          >
            Save
          </button>
